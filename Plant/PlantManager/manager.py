"""Long-running PlantManager: poll WORKORDER stream, prepare Floor, start sandboxes."""
...
from PlantManager.floor import (
    agents_worktree,
...
from PlantManager.pool import LOCAL_REDIS_URL, start_ticket_sandbox
...
class PlantManager:
    def __init__(self) -> None:
        ...
        self.consumer = os.environ.get(
            "PLANT_CONSUMER", f"plant-{os.getpid()}"
        )
    ...
    def _write_harness(
        ...
        *,
        ...
        key = harness_key(guid)
        redis.hset(
            key,
            mapping=harness_fields(
                ticket_id=ticket_id,
                status=status,
                error=error,
            ),
    ...
    def _fail_harness(
        ...
        *,
        ...
        """Publish FINISHED stream entry and delete LIVEHARNESS when sandbox fails."""
        key = harness_key(guid)
        redis.hset(
            key,
            mapping=harness_fields(
    ...
    def run_forever(self) -> None:
        ...
        log.info(
            "PlantManager polling stream %s (group=%s consumer=%s) floor=%s redis=%s",
            WORKORDER_STREAM,
            WORKORDER_GROUP,
            self.consumer,
            self.floor,
        ...
        log.info("PlantManager stopped")


def main() -> None:
    PlantManager().run_forever()

"""LiveHarness: LIVEHARNESS:<GUID> → WORKORDER lookup → agent → FINISHED:<REPO>."""
...
from LiveHarness.worktree_bind import WorktreeGitBind
...
# Load Plant/.env when present (does not override existing env vars).
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
load_dotenv(_PROJECT_ROOT / ".env", override=False)

log = logging.getLogger("live_harness")
...
def publish_finished(
    ...
    *,
    ...
    """XADD FINISHED:<repo> and delete the LIVEHARNESS hash."""
    ...
    redis.delete(harness)
    log.info("Pushed %s id=%s and deleted %s", stream, entry_id, harness)
...
def set_harness_status(
    ...
    *,
    ...
    redis.hset(
        key,
        mapping=harness_fields(ticket_id=ticket_id, status=status, error=error),
    )


class LiveHarness:
    """One-shot worker: resolve WORKORDER via LIVEHARNESS ticket_id, run agent, finish."""
    ...
    def run_once(self) -> int:
        key = harness_key(self.guid)
        redis = connect_redis(self.redis_url)

        log.info(
            "LiveHarness one-shot key=%s repo=%s ticket=%s cwd=%s",
            key,
            self.repo,
            self.ticket,
            self.workspace,
        ...
        # Prefer host paths from env (set by PlantManager); fall back to WORKORDER.wt.
        ...
        set_harness_status(redis, key, ticket_id=ticket_id, status="running")

"""CodeScope FE — point the cloud service at a repo, ask, read the answer.

This half is a client. Clones and the Cursor agent live on ``CODESCOPE_URL``.
The same HTTP path VoiceDeck uses, so a typed question and a spoken one hit
the same session.
"""
...
from CodeScopeManager.client import CodeScopeError, get_client
from CodeScopeManager.service import normalize_repo_url
...
class CodeScope:
    ...
    def _on_refresh(self, sender=None, app_data=None, user_data=None) -> None:
        if not self._ready or not self.session_id:
    ...
    def _on_question(self, sender=None, app_data=None, user_data=None) -> None:
        ...
        self._ask_queue.put((question, question_id))
    ...
    def _worker_loop(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                question, question_id = self._ask_queue.get_nowait()
            except queue.Empty:
                question = ""
                question_id = ""
            if question:
                self._do_ask(question, question_id)
                continue
    ...
    def _prepare(self, url: str) -> None:
        ...
        if not client.configured():
            ...
            self._prepared_url = url
            self._push(("conn", False))
            self._push(("status", "CODESCOPE_URL is not set", COLOR_RED))
            return
        model = DEFAULT_MODEL
        tag = self._tag("model")
        ...
        try:
            session = client.open_repo(url, model=model)
        except CodeScopeError as exc:
            self._ready = False
            self._prepared_url = url
            self._push(("conn", False))
        ...
        self._repo = session.get("repo") or repo
        self._prepared_url = url
        self._ready = True
        self._push(("conn", True))
        self._push(("status", f"Ready — {self._repo}", COLOR_GREEN))

    def _do_ask(self, question: str, question_id: str) -> None:
        client = get_client()
        try:
            for answer in client.ask(
                self.session_id,
                question,
    ...
    def _do_refresh(self) -> None:
        ...
        client = get_client()
        try:
            result = client.sync(self.session_id)
        except CodeScopeError as exc:
            self._push(("status", f"Sync failed: {exc}", COLOR_RED))
            return
        sha = (result.get("sha") or "")[:8]
        label = f"Synced {self._repo}" + (f" at {sha}" if sha else "")
        self._push(("status", label, COLOR_GREEN))

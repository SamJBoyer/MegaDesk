"""MissionControl Floor — canvas monitor for WORKORDER queue, live sandboxes, and Floor."""
...
_LIVE: dict[str, "MissionControlFloor"] = {}
...
class MissionControlFloor:
    """Read-only MissionControl pipeline monitor (does not consume WORKORDER)."""
    ...
    def _list_mission_control_containers(self) -> list[str]:
        try:
            proc = subprocess.run(
                [
                    "docker",
    ...
    def _scan_agent_handlers(self, client: redis.Redis) -> list[AgentHandlerRow]:
        ...
        while True:
            cursor, batch = client.scan(
                cursor=cursor, match=f"{AGENTHANDLER_PREFIX}*", count=AGENT_HANDLER_SCAN_COUNT
            )
            for key in batch:
                ...
                if not guid:
                    continue
                try:
                    parsed = parse_agent_handler(client.hgetall(key))
                except ValueError:
                    continue
    ...
    def _poll(self, force: bool = False) -> None:
        ...
        self._containers = self._list_mission_control_containers() if self._docker_ok else []
    ...
    def _refresh_widgets(self) -> None:
        ...
        if dpg.does_item_exist(dock):
            items = self._containers or ["(no mc-* containers)"]
    ...
    def build_ui(
        ...
        *,
        ...
        """Fill the host content parent with MissionControl Floor widgets."""
...
def main() -> None:
    raise SystemExit(
        "MissionControl FE is canvas-only. Drop it from the MegaDesk Catalog."
    )

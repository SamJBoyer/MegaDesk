...
VoiceDeck notepad tools (`new_document`, `add_text`, `switch_document`) publish here
and return immediately. The notepad FE applies the command to the open pad.

| Field | Required | Notes |
|---|---|---|
| `action` | yes | `create`, `append`, or `switch` |
| `title` | create / switch | Document / tab name |
| `text` | append | Text to append; optional on create |

## Code references

- `MegaDesk-Contracts/megadesk_contracts/wire/{code_scope,voice,cloud,factory,notepad}.py` — the definitions

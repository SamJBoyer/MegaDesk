...
- Nodes (`Nodes/`): productivity nodes installed via `pip install -e Nodes/<name>` (or `.[canvas]` where noted).
  ...
  - PromptImprover: FE + BE + tools node — a chat window that rewrites a rough prompt via one OpenAI Chat Completions call. VoiceDeck can call Revise My Prompt and speak the rewrite. Needs `OPENAI_API_KEY`.

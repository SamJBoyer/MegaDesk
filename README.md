...
- Nodes (`Nodes/`): productivity nodes installed via `pip install -e Nodes/<name>` (or `.[canvas]` where noted).
  ...
  - Notepad: FE-only tabbed pad. Notes save as `.txt` files that can be git-included and pushed to GitHub. VoiceDeck can create documents, add text, and switch the target tab.

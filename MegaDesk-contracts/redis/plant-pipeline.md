...
TicketDispatcher / MergeManager
        ...
        ▼
   WORKORDER (stream)
        │  XREADGROUP group=plant
        ▼
   PlantManager
        │  HSET + Docker sandbox
        ▼
   LIVEHARNESS:<GUID> (hash)
...
| Type | Stream |
| Key | `WORKORDER` |
| Consumer group | `plant` |
| Primary consumer | PlantManager |
...
The Redis stream entry id returned by `XADD` (e.g. `1712345678901-0`) is the **ticket_id** referenced by `LIVEHARNESS:<GUID>.ticket_id` and later echoed on `FINISHED:<REPO>`.
...
|----------|-------|
| Type | Hash |
| Key | `LIVEHARNESS:<GUID>` |
| Writer | PlantManager (create), LiveHarness (status updates) |
| Reader | LiveHarness |
...
Ticket payload (`ticket_name`, `instructions`, `model`, paths) is **not** stored on this hash. LiveHarness loads those from `WORKORDER` via `ticket_id`. Host absolute paths for the finished package are passed into the container as env (`HOST_WT`, `HOST_AGENT_DIR`).
...
| Producer | LiveHarness (and PlantManager error paths that publish finished) |
...
|--------|-------|----------|
| `WORKORDER` | `plant` | After PlantManager finishes handling the entry (success or handled failure) |
...
| Env | Meaning |
|-----|---------|
| `GUID` | Sandbox id → hash key `LIVEHARNESS:<GUID>` |

...
including per-layer member id lists). Each object gets its own GUID. When an FE
is dropped onto the canvas it is added to `members` and to the active layer
under `hierarchy.layers`.

Display Engine: parses `canvas.json` and displays MegaDesk members on the board.

Layout:

- Layer bar (bottom left): toggle lock/visibility; create, rename, remove layers.
...
| UI-5 | Layer control with visibility and lock toggles. | Must |

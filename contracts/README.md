...
These docs are the cross-module source of truth for key names, stream/hash field shapes, consumer groups, and related conventions. Implementation helpers live in module-local `redis_packets.py` files; when those diverge, treat this folder as the intended contract and reconcile the code.
...
| **MegaDesk canvas (`src/`)** | On canvas drop of a MegaDesk FE that also exposes a BE, `XADD`s `LAUNCHREQUEST` |

...
Notepad: FE-only canvas node — a tabbed pad of text notes saved as `.txt` files that can be git-included and pushed to GitHub. VoiceDeck can create documents, append text, and switch the target tab via `NOTEPAD:COMMAND`.
